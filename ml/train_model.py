...
def train_and_evaluate():
    ...
    frontend_img_dir = os.path.join(base_dir, "frontend", "img")
    
    os.makedirs(ml_dir, exist_ok=True)
    os.makedirs(frontend_img_dir, exist_ok=True)
    
    print("=== GeoMiner AI: Loading datasets ===")
    ...
    print("=== Calculating Mineral Potential Score target variable ===")
    ...
    print("=== Tuning Model via GridSearchCV ===")
    t0 = time.time()
    ...
    best_model_path = os.path.join(ml_dir, "best_model.pkl")
    joblib.dump(best_model, best_model_path)
    print(f"\nModel saved successfully at: {best_model_path}")
    
    # Generate Evaluation Charts in frontend/img/
    print("\n=== Generating evaluation plots in frontend/img/ ===")
    
    # 1. Predicted vs Actual
    plt.figure(figsize=(8, 6))
    sns.scatterplot(x=y_test, y=y_pred, alpha=0.5, color="#1e3a8a")
    plt.plot([0, 1], [0, 1], color="#ef4444", linestyle="--", linewidth=2)
    ...
    pred_vs_act_path = os.path.join(frontend_img_dir, "predicted_vs_actual.png")
    ...
    feat_imp_path = os.path.join(frontend_img_dir, "feature_importance.png")
    ...
    corr_heatmap_path = os.path.join(frontend_img_dir, "correlation_heatmap.png")
